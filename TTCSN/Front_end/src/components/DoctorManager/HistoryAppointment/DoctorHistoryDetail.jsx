...
const DoctorHistoryDetail = ({ history, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black/40 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-4  rounded-2xl w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between">
                    <h2 className="text-xl font-bold mb-4">Chi tiết lịch sử khám</h2>
                    <button onClick={onClose} className="mb-4 px-3 py-1 border rounded border-gray-300 cursor-pointer hover:bg-gray-100">Đóng</button>
                </div>
                <div className="space-y-2">
                    ...
                    <p><strong>Ngày bác sĩ xác nhận:</strong> {history.ngayBacSiXacNhan || "Chưa có"}</p>
                    ...
                    <p><strong>Lý do hủy:</strong> {history.lyDoHuy || "Chưa có"}</p>
                    <p><strong>Người hủy:</strong> {history.tenNguoiHuy || "Chưa có"}</p>
                    <p><strong>Ngày hoàn tiền:</strong> {history.ngayHoanTien || "Chưa có"}</p>
                    <p><strong>Số tiền hoàn:</strong> {history.soTienHoan || "Chưa có"}</p>
                    <p><strong>Lý do hoàn tiền:</strong> {history.lyDoHoanTien || "Chưa có"}</p>
                </div>
            </div>
        </div>
    );
